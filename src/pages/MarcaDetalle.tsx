...
import { ArrowLeft, Loader2, ChevronLeft, ChevronRight, Tag } from 'lucide-react';
...
const MarcaDetalle = () => {
  ...
  // Resolve brand canonical name from slug
  const brandName = brands.find(b => slugify(b) === slug) || slug?.replace(/-/g, ' ') || '';
  ...
  const { data, isLoading } = useCatalogProducts({
    ...
    brands: brandName ? [brandName] : undefined,
    sortBy,
    officialOnly: true,
  });
  ...
  const displayName = brands.find(b => slugify(b) === slug) || (slug ? slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : '');
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-muted-foreground">
            {isLoading ? 'Cargando...' : `${totalCount.toLocaleString('es-MX')} productos de esta marca`}
          </p>
        </motion.div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-muted-foreground text-lg mb-4">No se encontraron productos para esta marca.</p>
